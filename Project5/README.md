...
## Artifacts on GCS (submission outputs)

Bucket (example): `gs://unigap-prj5-raw/`

Recommended prefixes (audit-friendly):

- Raw dump (BSON + metadata):
  - `raw/glamira/ingest_date=2026-04-22/`
- Exports (derived outputs):
  - `exports/ip_locations/run_date=2026-04-22/`
  - `exports/products/run_date=2026-04-22/`

Quick checks (Cloud Shell / VM with gcloud):

```bash
# List what's actually on GCS (avoid "folder placeholder" confusion)
gcloud storage ls -l -r "gs://unigap-prj5-raw/exports/ip_locations/**"
gcloud storage ls -l -r "gs://unigap-prj5-raw/exports/products/**"

# Spot-check first rows of a CSV (replace filename with what `ls` shows)
gcloud storage cat "gs://unigap-prj5-raw/exports/ip_locations/run_date=2026-04-22/<ip_locations_file>.csv" | head -n 5
gcloud storage cat "gs://unigap-prj5-raw/exports/products/run_date=2026-04-22/<products_file>.csv" | head -n 5
```

Reference counts from the VM (example):

- `unique_ips`: 3,239,628
- `ip_locations`: 3,239,628
- `product_candidates` (unique products): 19,417
